...
mkdir -p /storage/observium-01/rrd
mkdir -p /storage/observium-01/logs
mkdir -p /storage/observium-01/mariadb
...
docker run -d --restart=always -h nuva-observium-01 --name=nuva-observium-01 \
    -p 8080:80 -p 2222:22 \
    --mount type=bind,source=/storage/observium-01/rrd,destination=/opt/observium/rrd,readonly=false \
    --mount type=bind,source=/storage/observium-01/logs,destination=/opt/observium/logs,readonly=false \
    --mount type=bind,source=/storage/observium-01/mariadb,destination=/var/lib/mysql,readonly=false \
